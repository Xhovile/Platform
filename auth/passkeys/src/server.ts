...
export async function createRegistrationOptions(
...
): Promise<PasskeyOptionsResult<PublicKeyCredentialCreationOptionsJSON>> {
  ...
  await ceremonyRepository.save({
    ...
    expiresAt: new Date(Date.now() + ttl(config)),
  });
...
export async function verifyRegistration(
  ...
  const savedCredential: PasskeyCredential = {
    id: credential.id,
...
export async function createAuthenticationOptions(
  ...
  ceremonyRepository: PasskeyCeremonyRepository,
  config: PasskeyServerConfig,
): Promise<PasskeyOptionsResult<PublicKeyCredentialRequestOptionsJSON>> {
...
export async function verifyAuthentication(
  ...
  credentialRepository: PasskeyCredentialRepository,
  config: PasskeyServerConfig,
): Promise<{ verified: boolean; credentialId?: string; userId?: string }> {
  const ceremony = await ceremonyRepository.consume(ceremonyId);
  if (
    !ceremony ||
  ...
  if (!credential || (ceremony.userId && credential.userId !== ceremony.userId)) {
    return { verified: false };
  }
